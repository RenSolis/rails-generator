...
def registry
  @registry ||= ask_with_default('What is your registry username?', 'rensolis')
end

def domain
  @domain ||= ask_with_default('What is the domain name of your server?', 'rensolis.dev')
end
...
after_bundle do
  ...
  run 'docker compose run app bin/rails db:create db:migrate db:seed'
